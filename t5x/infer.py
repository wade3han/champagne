  ...
  with gfile.GFile(path, 'w') as f:
    for i, inp in task_ds.enumerate().as_numpy_iterator():
      ...
      #   json_dict['aux'] = jax.tree_map(_json_compat, pred_aux)
        ...
        text = vocabulary.decode_tf(tf.constant(inp['text_inputs'])).numpy().decode('utf-8')
        example_id = inp['example_id'].numpy()
        image = (pred_aux['img'] + 1) / 2.0  # Undo pre-processings
        ...
        if len(image.shape) == 4:
          # create folder directory if not.
          image_folder_path = "%s/images/%s"%('coco_gen', example_id.decode("utf-8").split('.')[0])
          os.makedirs(image_folder_path, exist_ok=True)
          with open(image_folder_path + '/' + 'text.txt', 'w') as txt_f: txt_f.write(text)
          for i in range(image.shape[0]):
            im = Image.fromarray(image[i])
            im.save(image_folder_path + "/" + str(i) + '.png')
            vocabulary.decode_tf(tf.constant(predictions)).numpy()
        if output_ids:
          pred = _json_compat(tf.constant(predictions).numpy())
          # Truncate padding tokens.
  ...
  def infer_task(task: seqio.Task):
    ...
    def _write_chunk_and_canonicalize_ckpt(chunk: int, chunk_path: str,
      ...
      write_fn(chunk_path, inferences, task_ds, mode,
               task.output_features['text_targets'].vocabulary)
