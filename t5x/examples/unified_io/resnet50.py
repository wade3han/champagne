# From flax_models, modified to use our Conv/batch_norm layers
...
from t5x.examples.unified_io.batch_norm import batch_norm
...
class BasicBlock(nn.Module):
  ...
  def __call__(self, x, act, train=True):
    ...
    x = layers.Conv(features=self.features,
    ...
    if self.downsample:
      residual = layers.Conv(features=self.features,
                         kernel_size=(1, 1),
                         strides=(2, 2),
...
class Bottleneck(nn.Module):
  ...
  def __call__(self, x, act, train=True):
    ...
    x = layers.Conv(features=self.features * self.expansion,
                kernel_size=(1, 1),
                strides=(1, 1),
    ...
    if self.downsample:
      residual = layers.Conv(features=self.features * self.expansion,
                         kernel_size=(1, 1),
...
class ResNet(nn.Module):
  ...
  def __call__(self, x, train=True):
    ...
    # x = layers.DenseGeneral(features=num_classes,
