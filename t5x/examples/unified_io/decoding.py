...
def block_ngrams(ngram_size: int,
                 logits: jnp.ndarray,
                 sequences: jnp.ndarray,
                 i: jnp.DeviceArray) -> jnp.ndarray:
  batch_size, _, vocab_size = logits.shape
  next_tokens = jnp.arange(vocab_size, dtype=sequences.dtype)  # [vocab_size]
  next_tokens = jnp.tile(next_tokens[None, None], [batch_size, 1, 1])  # [batch, 1, vocab_size]
  assert len(next_tokens.shape) == 3
  tiled_sequences = jnp.tile(sequences[:, :, None], [1, 1, vocab_size])  # [batch, length, vocab_size]
  next_hyps = jax.lax.dynamic_update_slice(tiled_sequences, next_tokens,
                                           (0, i + 1, 0))
  ngram_postfix = jax.lax.dynamic_slice(next_hyps,
                                        (0, i - ngram_size + 2, 0),
                                        (batch_size, ngram_size, vocab_size))
  init_state = (
    logits,
    0,  # start index to get the window
  )

  def loop_cond_fn(state) -> bool:
    _, ind = state
    return ind + ngram_size <= i + 1
  def loop_body_fn(state):
    _logits, ind = state
    ngram_window = jax.lax.dynamic_slice(tiled_sequences,
                                         (0, ind, 0),
                                         (batch_size, ngram_size, vocab_size))
    is_ngram_same = jnp.all(ngram_window == ngram_postfix, axis=1, keepdims=True)  # [batch, 1, vocab_size]
    new_logits = jnp.where(is_ngram_same, jnp.full_like(_logits, NEG_INF), _logits)
    return new_logits, ind + 1
...
def context_block_ngrams(ngram_size: int,
                         logits: jnp.ndarray,
                         sequences: jnp.ndarray,
  ...
  # input dialogue contexts
  contexts = flat_batch_beam_expand(contexts, batch_size // contexts.shape[0])
  context_length = contexts.shape[-1]
  contexts = jnp.tile(contexts[:, :, None], [1, 1, vocab_size])  # [batch, L, vocab_size]
  ...
  ngram_postfix = jax.lax.dynamic_slice(next_hyps,
                                        (0, i - ngram_size + 2, 0),
                                        (batch_size, ngram_size, vocab_size))  # [batch, ngram_size, vocab_size]
  init_state = (
    logits,
    0,  # start index to get the window
  )
  ...
  def loop_cond_fn(state) -> bool:
    _, ind = state
    return ind + ngram_size <= context_length

  def loop_body_fn(state):
    _logits, ind = state
    ngram_window = jax.lax.dynamic_slice(contexts,
                                         (0, ind, 0),
                                         (batch_size, ngram_size, vocab_size))
    is_ngram_same = jnp.all(ngram_window == ngram_postfix, axis=1, keepdims=True)  # [batch, 1, vocab_size]
    is_window_all_nonpad = jnp.all(ngram_window != 0, axis=1, keepdims=True)  # [batch, 1, vocab_size]
    new_logits = jnp.where(jnp.logical_and(is_ngram_same, is_window_all_nonpad),
                           jnp.full_like(_logits, NEG_INF),
                           _logits)
    return new_logits, ind + 1
...
def temperature_sample(
    ...
    ngram_block_size: Optional[int] = None,
    context_ngram_block_size: Optional[int] = None,
    min_beam_length: Optional[int] = None,
    input_texts: Optional[jnp.ndarray] = None,
  ...
  if ngram_block_size is not None and context_ngram_block_size is not None and min_beam_length is not None:
    expanded_decodes, expanded_log_prob, expanded_all_logprob = _temperature_sample_single_trial_for_dialogue(
      expanded_inputs,
      expanded_cache,
      tokens_to_logits,
      eos_id,
    ...
    assert ngram_block_size is None and context_ngram_block_size is None and min_beam_length is None
...
def _temperature_sample_single_trial_for_dialogue(
    inputs: jnp.ndarray,
    cache: Mapping[str, jnp.ndarray],
    tokens_to_logits: Callable[[jnp.ndarray, Mapping[str, jnp.ndarray]],
                               Tuple[jnp.ndarray, Mapping[str, jnp.ndarray]]],
    eos_id: int,
    prng_key: jnp.ndarray,
    ngram_block_size: int,
    context_ngram_block_size: int,
    min_beam_length: int,
    temperature: float = 1.0,
    topk: int = 20,
    topp: float = 0.0,
  ...
  def sampling_loop_body_fn(state: SamplingLoopState) -> SamplingLoopState:
    ...
    # mask out image vocab part because it is not used for dialogue
    total_vocab_size = logits.shape[-1]
    logits = jnp.where(
      jnp.logical_or(
        jnp.tile(jnp.arange(total_vocab_size, dtype=jnp.int32)[None, :], (logits.shape[0], 1)) >= 33100,
        jnp.tile(jnp.arange(total_vocab_size, dtype=jnp.int32)[None, :], (logits.shape[0], 1)) == 2),
      jnp.full_like(logits, NEG_INF),
      logits,
    )
    # call(lambda x: print(f"Logits: {x[0, :10]}, {x[0, -10:]}"), logits)
    logits = block_ngrams(ngram_size=ngram_block_size,
                          logits=logits[:, None, :],
                          sequences=sequences,
                          i=i[0])
    # call(lambda x: print(f"Logits after block ngrams: {x[0, :10]}, {x[0, -10:]}"), logits)
    logits = context_block_ngrams(ngram_size=context_ngram_block_size,
                                  logits=logits[:, None, :],
                                  sequences=sequences,
                                  contexts=input_texts,
                                  i=i[0])
    # call(lambda x: print(f"Logits after context block ngrams: {x[0, :10]}, {x[0, -10:]}"), logits)
    # if current timestep is before min beam length, then mask eos id
    before_min_beam_length = jnp.expand_dims(i <= min_beam_length, axis=1)  # [batch, 1]
    eos_mask = jax.nn.one_hot(1, logits.shape[1], dtype=logits.dtype) * NEG_INF
    eos_mask = jnp.tile(eos_mask[None, :], [logits.shape[0], 1]) * before_min_beam_length  # [batch, vocab]
    logits = logits + eos_mask
    # call(lambda x: print(f"Logits after eos mask: {x[0, :10]}, {x[0, -10:]}"), logits)
    ...
    if topk:
      # Get top-k logits and their indices, sample within these top-k tokens.
      # call(lambda x: print(f"Logits before topk: {x[0, :10]}, {x[0, -10:]}"), logits)
      topk_logits, topk_idxs = lax.top_k(logits, topk)
      # call(lambda x: print(f"topk: {x[0, :4]}"), topk_idxs)
      topk_token = jnp.expand_dims(
        random.categorical(rng1, topk_logits / temperature).astype(jnp.int32),
        axis=-1)
      # Return the original indices corresponding to the sampled top-k tokens.
...
def beam_search(inputs: jnp.ndarray,
  ...
  def beam_search_loop_body_fn(state: BeamState) -> BeamState:
    ...
    total_vocab_size = flat_logits.shape[-1]
    flat_logits = jnp.where(
      jnp.logical_or(
        jnp.tile(jnp.arange(total_vocab_size, dtype=jnp.int32)[None, :], (flat_logits.shape[0], 1))
          >= total_vocab_size - image_vocab_size,
        jnp.tile(jnp.arange(total_vocab_size, dtype=jnp.int32)[None, :], (flat_logits.shape[0], 1)) == 2),
      jnp.full_like(flat_logits, NEG_INF),
      flat_logits,
    )

    if ngram_block_size is not None:
      flat_logits = block_ngrams(ngram_size=ngram_block_size,
                                 logits=flat_logits[:, None, :],
                                 sequences=jnp.reshape(state.live_seqs, [batch_size * beam_size, -1]),
                                 i=state.cur_index)
    if context_ngram_block_size is not None:
      flat_logits = context_block_ngrams(ngram_size=context_ngram_block_size,
                                         logits=flat_logits[:, None, :],
                                         sequences=jnp.reshape(state.live_seqs, [batch_size * beam_size, -1]),
                                         contexts=input_texts,
                                         i=state.cur_index)

    if min_beam_length is not None:
      # if current timestep is before min beam length, then mask eos id
      cur_index = jnp.array([state.cur_index])
      before_min_beam_length = jnp.expand_dims(cur_index <= min_beam_length, axis=1)  # [batch, 1]
      eos_mask = jax.nn.one_hot(1, flat_logits.shape[1], dtype=flat_logits.dtype) * NEG_INF
      eos_mask = jnp.tile(eos_mask[None, :], [flat_logits.shape[0], 1]) * before_min_beam_length  # [batch, vocab]
      flat_logits = flat_logits + eos_mask
