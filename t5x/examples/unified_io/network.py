...
from t5x.examples.unified_io.resnet50 import ResNet50
...
class Encoder(nn.Module):
  ...
  def __call__(self,
    ...
    if image_encoder_inputs.ndim == 4:
      if cfg.visual_backbone_type == "resnet50":
        img_emb = ResNet50(
            pretrained='imagenet', 
            output='activations', 
            normalize=False, 
            dtype=cfg.dtype)(image_encoder_inputs, train=False)
            
        img_emb = img_emb[cfg.visual_backbone_feature]
        bs, h, w, dim = img_emb.shape
        img_emb = img_emb.reshape((bs, h*w, dim))
      else:
    ...
    if image_encoder_inputs.ndim == 4 and img_emb.shape[1] != cfg.encoder_max_image_length:
