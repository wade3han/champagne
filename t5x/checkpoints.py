...
from t5x import checkpoint_utils, checkpoint_importer
...
class Checkpointer(object):
  ...
  def restore(
    ...
    if fallback_state is None:
      logging.info('No fallback state provided, restoring all parameters.')
      ...
      logging.info('Restoring only parameters matched by checkpoint.')
    ...
    for key in restore_parameter_infos_flat.keys():
      logging.debug('Parameter infos: %s', key)

    ckpt_parameter_infos_flat = state_utils.flatten_state_dict(
        ckpt_state_dict)
    for key in ckpt_parameter_infos_flat.keys():
      logging.debug('Restoring key from ckpt: %s', key)
