...
class _AdafactorHyperParams:
  ...
  trainable_param_name: Optional[str] = None
...
class Adafactor(OptimizerDef):
  ...
  def __init__(self,
    ...
    hyper_params = _AdafactorHyperParams(
        ...
        global_norm_clip_threshold, max_parameter_scale, skip_nan_updates, trainable_param_name)
  ...
  def apply_gradient(self, hyper_params, params, state, grads):
    ...
    # debug & logging to check if freezing works
    logging.info(f'debugging: if freezing works?')
    if self.hyper_params.trainable_param_name and not self.print_once:
      monitor_grads = []
      for k in params_paths:
        if self.hyper_params.trainable_param_name not in k:
          monitor_grads.append((k, grads_flat[k]))
      logging.info(f'non-trainable params (10 examples): {monitor_grads[:10]}')
      logging.info(f'non-trainable params all keys: {[k[0] for k in monitor_grads]}')
      self.print_once = True

    # if self.hyper_params.trainable_param_name:
    #   grads_flat = [grads_flat[k] if self.hyper_params.trainable_param_name in k
    #                 else 0.0 * grads_flat[k] for k in params_paths]
    # else:
    ...
    if self.hyper_params.trainable_param_name:
      new_params_flat = {k: v_new if self.hyper_params.trainable_param_name in k else v_old for k, v_new, v_old in
                         zip(params_paths, new_params_flat, params_flat)}
      new_states_flat = {k: v_new if self.hyper_params.trainable_param_name in k else v_old for k, v_new, v_old in
                         zip(params_paths, new_states_flat, states_flat)}
    else:
