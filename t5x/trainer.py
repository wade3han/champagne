...
class BaseTrainer(abc.ABC):
  ...
  def __init__(self, model: models.BaseModel,
               ...
               train_state_axes: Any, rng: Rng, use_wandb=False):
    """Trainer constructor.
  ...
  def train(self,
    ...
    with self._train_state_mutex:
      ...
      for step_num in range(start_step, start_step + num_steps):
          if metrics:
            lr = jax.numpy.asarray(metrics['learning_rate'].compute())
            lr = f"{lr:0.4f}"
            try:
              loss = jax.numpy.asarray(metrics['loss'].compute() + metrics['image_loss'].compute())
            except KeyError:
              loss = jax.numpy.asarray(metrics['loss'].compute())
            loss = f"{loss:0.4f}"
          else:
            lr = None
            loss = None
          logging.log_every_n_seconds(logging.INFO, f"Training: step {step_num}, lr {lr}, loss {loss}", 10)
  ...
  def eval(
    ...
    for iter_name, batch_iter in batch_iters.items():
      ...
      if jax.process_index() == 0 and self.use_wandb:
        import wandb
        metric_dict = {}
        for k, v in metrics.items():
          if isinstance(v, Time):
            continue
          val = v.compute_value()
          if isinstance(val, clu.values.Scalar):
            val = val.value
            if isinstance(val, jnp.ndarray):
              val = np.array(val)
            # Eval prefix so this never overlaps with Evaluator metrics
            metric_dict["eval-" + k] = val
        metric_dict["eval-step"] = train_state.step
...
def accumulate_grads_microbatched(
  ...
  if num_microbatches is None or num_microbatches <= 1:
    ...
    del new_dropout_rng
  grad_accum = jax.tree_map(lambda x: jnp.nan_to_num(x, copy=False), grad_accum)
